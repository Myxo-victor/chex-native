/**
 * Chex Native Core (v6.0.0)
 * A small JavaScript API for rendering native Android and iOS UI.
 * `venjs` remains available as a compatibility alias for existing apps.
 */
...
const venjs = {
  ...
  _dispatchEvent: (eventId, payload) => {
    ...
    if (typeof handler !== 'function') {
      console.warn(`Chex: No handler found for event id ${eventId}.`);
      return;
    }
    ...
    try {
      ...
      console.error('Chex: Error while executing event handler.', error);
    } finally {
  ...
  _transmit: (uiTree) => {
    ...
    console.warn('Chex: No native engine detected. Rendering to console for debugging.');
    console.dir(uiTree);
  },

  _renderRoot: () => {
    if (typeof venjs._rootComponent !== 'function') {
      const err = new Error('Chex: no root component mounted. Call venjs.mount(App) first.');
  ...
  _deviceRequest: (action, params = {}) => new Promise((resolve, reject) => {
    if (!action || typeof action !== 'string') {
      reject(new Error('Chex: device request requires an action string.'));
      return;
    }
    ...
    if (!venjs._sendDeviceRequest(requestPayload)) {
      delete venjs._persistentEventHandlers[eventId];
      reject(new Error('Chex: no native device bridge detected.'));
    }
  }),
  ...
  clipboard: {
    setText: (text) => venjs._deviceRequest('setClipboard', { text: String(text ?? '') }),
    getText: () => venjs._deviceRequest('getClipboard')
      .then((result) => String((result && result.text) || ''))
  },

  onShake: (handler, spec = {}) => {
    if (typeof handler !== 'function') {
      throw new Error('Chex.onShake requires a function.');
    }
    ...
    if (!venjs._sendDeviceRequest(startPayload)) {
      delete venjs._persistentEventHandlers[eventId];
      throw new Error('Chex: no native device bridge detected.');
    }
  ...
  _subscribeNotificationListener: (kind, handler) => {
    if (typeof handler !== 'function') {
      throw new Error('Chex.notifications listener requires a function.');
    }
    const normalizedKind = String(kind || '').trim();
    if (!normalizedKind) {
      throw new Error('Chex.notifications listener requires a kind.');
    }
  ...
  use: (plugin, options = {}) => {
    ...
    if (!target || typeof target.install !== 'function') {
      throw new Error('Chex.use requires a plugin with an install(venjs, options) function.');
    }
    ...
    const notify = () => {
      listeners.forEach((listener) => {
        try {
          ...
          console.error('Chex store listener error:', error);
        }
      });
    };
  ...
  createRouter: (routes = {}, options = {}) => {
    ...
    if (typeof window !== 'undefined') {
      const handleNativeBack = () => back();
      window.__chexHandleNativeBack = handleNativeBack;
      // Kept for applications compiled against the old bridge.
      window.__venjsHandleNativeBack = handleNativeBack;
    }
  ...
  mount: (arg1, arg2, arg3) => {
    ...
    if (typeof component !== 'function') {
      throw new Error('Chex.mount requires a component function.');
    }
...
if (typeof window !== 'undefined') {
  // Chex is the public API.  Its call shape deliberately matches the web
  // runtime: `div([text('Hello')])`, `button('Save')`, and `signal.value`.
  // The native bridge continues to receive the compact serialized VNode tree.
  const Chex = (() => {
    const nativeTags = {
      h1: { tag: 'text', fontSize: 30, fontWeight: '700' },
      h2: { tag: 'text', fontSize: 24, fontWeight: '700' },
      h3: { tag: 'text', fontSize: 20, fontWeight: '600' },
      p: { tag: 'text', fontSize: 16 },
      span: { tag: 'text', fontSize: 16 },
      label: { tag: 'text', fontSize: 14 },
      header: { tag: 'div' },
      main: { tag: 'div' },
      section: { tag: 'div' },
      footer: { tag: 'div' },
      article: { tag: 'div' },
      row: { tag: 'div', flexDirection: 'row' },
      column: { tag: 'div', flexDirection: 'column' }
    };

    const withDefaults = (name, first, rest) => {
      const definition = nativeTags[name] || { tag: name };
      const hasProps = venjs._isPropsObject(first);
      const supplied = hasProps ? first : {};
      const style = { ...(definition.fontSize ? { fontSize: definition.fontSize } : {}),
        ...(definition.fontWeight ? { fontWeight: definition.fontWeight } : {}),
        ...(definition.flexDirection ? { flexDirection: definition.flexDirection } : {}),
        ...((supplied && supplied.style) || {}) };
      if (hasProps) {
        return venjs.createElement(definition.tag, { ...supplied, style }, ...rest);
      }
      return venjs.createElement(definition.tag, { style }, first, ...rest);
    };

    const api = {
      ...venjs,
      signal(initialValue) {
        const state = venjs.state(initialValue);
        return {
          get value() { return state.get(); },
          set value(next) { state.set(next); },
          peek: () => state.get(),
          get: () => state.get(),
          set: (next) => state.set(next)
        };
      },
      // Native apps have one root; accepting the web-style `render(App)` and
      // `render(container, App)` keeps shared Chex components source-compatible.
      render(first, second, options) {
        const component = typeof first === 'function' ? first : second;
        return venjs.mount(component, options);
      },
      mount: (...args) => venjs.mount(...args),
      h: (...args) => venjs.createElement(...args)
    };

    Object.keys(nativeTags).forEach((name) => {
      api[name] = (first, ...rest) => withDefaults(name, first, rest);
    });
    ['div', 'text', 'button', 'image', 'input', 'checkbox', 'icon', 'activityIndicator', 'a', 'select'].forEach((name) => {
      api[name] = (...args) => venjs.createElement(name, ...args);
    });
    return api;
  })();
  ...
  window.venjs = venjs;
  window.venjsX = venjs;
  // Backward compatibility for legacy venjsX apps.
  window.venX = venjs;
