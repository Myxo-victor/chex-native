...
class VenjsXEngine: NSObject, WKScriptMessageHandler, WKUIDelegate, CLLocationManagerDelegate {
  ...
  private func handleDeviceRequest(_ jsonString: String) {
    ...
    case "setClipboard":
      handleSetClipboard(eventId: eventId, params: params)
    case "getClipboard":
      handleGetClipboard(eventId: eventId)
    case "startShake":
  ...
  private func handleNativeLog(params: [String: Any]) {
    ...
    print("Chex[\(level)]: \(message)")
  }

  private func handleSetClipboard(eventId: Int, params: [String: Any]) {
    DispatchQueue.main.async {
      let text = (params["text"] as? String) ?? ""
      UIPasteboard.general.string = text
      self.emitDeviceOk(eventId: eventId, extra: ["text": text])
    }
  }

  private func handleGetClipboard(eventId: Int) {
    DispatchQueue.main.async {
      self.emitDeviceOk(eventId: eventId, extra: ["text": UIPasteboard.general.string ?? ""])
    }
  }
  ...
  private func deviceRootDir() -> URL? {
    ...
    let dir = base.appendingPathComponent("Chex", isDirectory: true)
