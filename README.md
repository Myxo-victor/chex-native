...
Tooling lives in `venX/` and is optional but recommended.

`venX/js/Chex.js` is the canonical runtime source. The CLI keeps the native shells’ runtime copies in sync automatically.
...
```js
const createStoragePlugin = require('./venX/plugins/venjs-plugin-storage');
...
- Android (FCM):
  - Put `google-services.json` in `venX/android/app/`.
  ...
  - Notification icon: set a dedicated small icon (recommended) in `venX/android/app/src/main/AndroidManifest.xml`:
    ```xml
    <meta-data
